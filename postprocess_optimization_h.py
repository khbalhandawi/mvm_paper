...
# base_folder = os.path.join('data','strut_fea_h','opt')
# man = get_man_combined()
# man_name = 'strut_comb'
# man.load(man_name,folder=base_folder)

# get the man object and load it
base_folder = os.path.join('data','strut_h','opt')
man = get_man()
man_name = 'strut_s'
man.load(man_name,folder=base_folder)
...
############################################################
# Optimization sweep
h_thicks = np.linspace(15.0,20.0,6)
n_designs = len(h_thicks)
n_epochs = 1000
#---------------------------------------------------
# Evaluate all the different designs
for i,h in enumerate(h_thicks):

    man.nominal_design_vector = np.array([h,0.0])
    man.reset()
    man.reset_outputs()
...
for i,(I,A,h) in enumerate(zip(I_list,A_list,h_thicks)):
    i += 8
    # Extract x and y
...
optimum_mean = np.array([h_thicks[np.argmax(mean_d)], np.max(mean_d)])
optimum_median = np.array([h_thicks[np.argmax(median_d)], np.max(median_d)])
optimum_unnormalized = np.array([h_thicks[np.argmax(distances_mean)], np.max(distances_mean)])
#---------------------------------------------------
# Plot sweep
...
a1 = ax.plot(h_thicks, distances_mean, color=color, linewidth = 1.0)
# a2 = ax.fill_between(h_thicks, lower_p, upper_p, color=color, alpha=0.2)
a3 = ax.axvline(optimum_unnormalized[0], color='k', linestyle='--', linewidth=0.5)
